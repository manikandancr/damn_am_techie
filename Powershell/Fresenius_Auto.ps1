...
if ($null -eq $lastruninfo.execution_id ) { write-host "No previous run updated. The process will continue.." }
  ...
  $lastrundate = [regex]::Match($firstline, '[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}').value

  $value = [datetime]::ParseExact($lastrundate, 'yyyy-MM-dd HH:mm:ss', $null)
...
for ($i = 1 ; $i -le $step; $i++) {
  ...
  $value = [datetime]::ParseExact([regex]::Match($lastline, '[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}').value, 'yyyy-MM-dd HH:mm:ss', $null)
